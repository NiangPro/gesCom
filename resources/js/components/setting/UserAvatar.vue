...
export default {
    ...
    data(){
        return {
            image:null,
            imageprev:`/storage/images/`+this.userEditing.avatar
        }
    },
    methods:{
        imageSelected(e){
        ...
        profilUpload(){
            if(this.image !== null){
                ...
                data.append('userId', this.userEditing.id);

                axios.post('/api/userAvatar', data)
                .then(response => this.$emit('avatarUploaded', response.data))
